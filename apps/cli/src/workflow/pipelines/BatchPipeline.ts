...
export class BatchPipeline<TInput, TOutput> extends EventEmitter {
  ...
  async process(inputs: TInput[]): Promise<TOutput[]> {
    ...
    this.logger.info(`Starting pipeline ${this.config.name} with ${String(inputs.length)} inputs`);
    ...
    for (const chunk of chunks) {
      const chunkPromises = chunk.map(async (input, index) => {
        ...
        try {
          ...
          this.stats.failed++;
          this.logger.error(
            `Failed to process input ${String(index)}: ${error instanceof Error ? error.message : String(error)}`
          );
      ...
      const validResults = chunkResults.filter((result): result is Awaited<TOutput> => result !== null);
    ...
    this.logger.info(
      `Pipeline ${this.config.name} completed. ${String(this.stats.successful)}/${String(this.stats.processed)} successful`
    );
  ...
  private async processInput(input: TInput, index: number): Promise<TOutput> {
    ...
    for (const [stageIndex, stage] of this.stages.entries()) {
      this.logger.debug(`Processing input ${String(index)} through stage ${String(stageIndex + 1)}: ${stage.name}`);
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          this.logger.debug(`Stage ${stage.name} completed in ${String(stageTime)}ms`);
          ...
          this.logger.warn(
            `Stage ${stage.name} failed (attempt ${String(retryCount)}/${String(maxRetries + 1)}): ${error instanceof Error ? error.message : String(error)}`
          );
