...
export class TagEditor {
  ...
  public async reviewTags(
    ...
    const reviewTags = sortedTags
      .filter(tag => tag.confidence < (this.options.minConfidence || 0.7) && tag.confidence >= (this.options.reviewThreshold || 0.5))
      .map(tag => tag.name);
   ...
   */
  public async promptForMergeStrategy(existingTags: string[], newTags: string[]): Promise<'append' | 'replace' | 'merge' | 'skip'> {
