...
export class TagManagementView extends ItemView {
  ...
  private renderTagsList(container: HTMLElement, filterTerm = ''): void {
    ...
    const filteredTags = filterTerm ? tags.filter((tag) => tag.name.toLowerCase().includes(filterTerm)) : tags;

    // No tags message
    if (filteredTags.length === 0) {
      container.createEl('p', {
        text: filterTerm ? 'No matching tags found.' : 'No tags found in your vault.',
      });
      return;
    }
  ...
  private getMostUsedTag(tags: { id: string; name: string; count: number }[]): string {
    ...
    const mostUsedTag = tags.reduce((prev, current) => (prev.count > current.count ? prev : current));
